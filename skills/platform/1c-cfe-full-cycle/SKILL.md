...
5. **Собственные объекты** (`/meta-compile`): если нужны новые реквизиты/ТЧ/модули
   - после compile: `reference/extension-cfe-checklist.md` + `/cfe-validate` (проверка 14)
...
- После `/meta-compile` в расширении — сверить `ConfigDumpInfo.xml` (см. `1c-meta-compile/reference/extension-cfe-checklist.md`)
- **Не править `Rights.xml` вручную** для новых РС/Enum до первой успешной загрузки (риск зависания на ~78%) — см. `1c-role-rights.mdc`
